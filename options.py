...
def add_general_options(
    ...
    **kwargs
    ...
    if single_task and generative:
        parser.add_argument(
            "--save-all", action="store_true", help="also store conv- and deconv-layers"
        )
    if not only_MNIST:
        parser.add_argument(
            "--convE-stag",
    ...
    parser.add_argument(
        "--no-gpus", action="store_false", dest="cuda", help="don't use GPUs"
    )
    parser.add_argument(
        "--data-dir",
        type=str,
...
def add_eval_options(
    ...
    **kwargs
    ...
    eval.add_argument(
        "--pdf",
        action="store_true",
        help="generate pdf with plots for individual experiment(s)",
    )
    eval.add_argument(
    ...
    if compare_code == "none":
        ...
        eval.add_argument(
            "--acc-log",
            type=int,
            default=None if single_task else 500,
            metavar="N",
    ...
    eval.add_argument(
        ...
        type=int,
        default=1024,
        help="# samples for evaluating accuracy (visdom-plots)",
    )
    ...
    if generative:
        ...
        eval.add_argument(
            "--no-samples",
            action="store_true",
            help="don't plot generated/reconstructed images",
        )
        if not only_MNIST:
            eval.add_argument(
                "--eval-tag",
...
def add_task_options(
    parser, only_MNIST=False, single_task=False, compare_code="none", **kwargs
):
    ...
    task_params.add_argument(
        "--experiment", type=str, default=task_default, choices=task_choices
    )
    if not single_task:
        task_params.add_argument(
    ...
    if not single_task and compare_code == "none":
        task_params.add_argument(
            "--only-last", action="store_true", help="only train on last task / episode"
        )
    return parser
...
def add_model_options(
    parser, only_MNIST=False, generative=False, single_task=False, **kwargs
):
    ...
    if not only_MNIST:
        ...
        model.add_argument(
            "--depth",
            type=int,
            default=5 if single_task else None,
    ...
    model.add_argument(
        "--fc-units",
        type=int,
        default=2000 if single_task else None,
        metavar="N",
    ...
    model.add_argument(
        ...
        type=int,
        metavar="N",
        help="# of hidden units final layer (default: fc-units)",
    )
    ...
    if generative:
        model.add_argument(
            ...
            type=int,
            default=100,
            help="size of latent representation (if feedback, def=100)",
        )
    return parser
    ...
    **kwargs
    ...
    if single_task:
        ...
        iter_epochs.add_argument(
            ...
            type=int,
            metavar="N",
            help='max # of iterations (replaces "--epochs")',
        )
    else:
        train_params.add_argument(
            "--iters", type=int, help="# batches to optimize main model"
        )
    train_params.add_argument(
        "--lr",
        type=float,
        default=0.0001 if single_task else None,
        help="learning rate",
    )
    train_params.add_argument(
        "--batch", type=int, default=256 if single_task else None, help="batch-size"
    )
    train_params.add_argument(
        "--init-weight", type=str, default="standard", choices=["standard", "xavier"]
    )
    train_params.add_argument(
        "--init-bias", type=str, default="standard", choices=["standard", "constant"]
    )
    if not single_task and compare_code not in ("replay"):
        train_params.add_argument(
            "--reinit",
            action="store_true",
            help="reinitialize networks before each new task",
        )
    if not only_MNIST:
        if compare_code in ("none"):
            train_params.add_argument(
        ...
        if compare_code in ("none") and generative:
            train_params.add_argument(
                "--pre-convD", action="store_true", help="use pretrained convD-layers"
            )
        if compare_code in ("none"):
            train_params.add_argument(
                "--freeze-convE",
                action="store_true",
                help="freeze parameters of convE-layers",
            )
        if compare_code in ("none") and generative:
            train_params.add_argument(
                "--freeze-convD",
...
def add_VAE_options(parser, only_MNIST=False, **kwargs):
    ...
    VAE.add_argument(
        ...
        default=1.0,
        dest="rcl",
        help="weight of recon-loss (def=1)",
    )
    VAE.add_argument(
        "--variat-weight",
        type=float,
...
def add_replay_options(parser, only_MNIST=False, compare_code="none", **kwargs):
    ...
    if compare_code not in ("replay"):
        replay.add_argument(
            ...
            type=int,
            metavar="N",
            help="batch-size for replay (default: batch)",
        )
    ...
    if not only_MNIST:
        replay.add_argument(
            "--g-depth",
            type=int,
            help="[depth] in generator (default: same as classifier)",
        )
    replay.add_argument(
        "--g-fc-lay",
        type=int,
    ...
    if compare_code not in ("replay"):
        ...
        replay.add_argument(
            "--g-h-dim",
            type=int,
            help="[h_dim] in generator (default: same as classifier)",
        )
    replay.add_argument(
        "--g-z-dim",
        type=int,
        default=100,
        help="size of generator's latent representation (def=100)",
    )
    ...
    replay.add_argument(
        ...
        type=int,
        dest="g_iters",
        help="# batches to optimize generator (def=[iters])",
    )
    replay.add_argument(
...
def add_bir_options(parser, only_MNIST=False, compare_code="none", **kwargs):
    ...
    if compare_code in ("none"):
        BIR.add_argument(
            "--brain-inspired",
            action="store_true",
            help="select defaults for brain-inspired replay",
        )
    ...
    BIR.add_argument(
        ...
        default=1.0,
        dest="pl",
        help="(FB) weight of prediction loss (def=1)",
    )
    # -where on the VAE should the softmax-layer be placed?
    BIR.add_argument(
        "--classify", type=str, default="beforeZ", choices=["beforeZ", "fromZ"]
    )
    ...
    if compare_code in ("none"):
        ...
        BIR.add_argument(
            "--per-class",
            action="store_true",
            help="if selected, each class has own modes",
        )
    BIR.add_argument(
        "--n-modes",
        type=int,
    ...
    BIR.add_argument(
        ...
        type=str,
        metavar="TYPE",
        help="decoder-gates: based on tasks or classes?",
    )
    ...
    if compare_code in ("all"):
        BIR.add_argument(
            ...
            type=float,
            metavar="PROP",
            help="decoder-gates: masking-prop for BI-R + SI",
        )
        BIR.add_argument(
            "--dg-c", type=float, metavar="C", help="SI hyperparameter for BI-R + SI"
        )
    # -hidden replay
    if (not only_MNIST) and compare_code in ("none"):
        BIR.add_argument(
            "--hidden",
            action="store_true",
            help="replay at 'internal level' (after conv-layers)",
        )
    return parser
...
def add_allocation_options(parser, compare_code="none", **kwargs):
    ...
    if compare_code in ("none"):
        cl.add_argument(
            "--ewc", action="store_true", help="use 'EWC' (Kirkpatrick et al, 2017)"
        )
    ...
    if compare_code in ("none"):
        cl.add_argument(
            "--online", action="store_true", help="--> EWC: perform 'online EWC'"
        )
    elif compare_code in ("all"):
        cl.add_argument(
            "--o-lambda", type=float, help="--> online EWC: regularisation strength"
        )
    if not compare_code in ("hyper"):
        cl.add_argument(
            "--gamma",
...
def set_defaults(
    ...
    **kwargs
    ...
    args.normalize = (
        args.normalize
        if args.experiment in ("CIFAR10", "CIFAR100", "fruits360")
        else False
    )
    args.augment = args.augment if args.experiment in ("CIFAR10", "CIFAR100") else False
    if hasattr(args, "depth"):
        args.depth = (
            (5 if args.experiment in ("CIFAR10", "CIFAR100", "fruits360") else 0)
    ...
    if hasattr(args, "dg_type"):
        args.dg_type = (
            ("task" if args.experiment == "permMNIST" else "class")
    ...
    if not single_task:
        args.tasks = (
            (
                ...
                else (10 if args.experiment in ("CIFAR100", "fruits360") else 100)
            )
            if args.tasks is None
            else args.tasks
        )
        args.iters = (
            (5000 if args.experiment in ("CIFAR100", "fruits360") else 2000)
        ...
        args.batch = (
            (128 if args.experiment == "splitMNIST" else 256)
            if args.batch is None
            else args.batch
        )
        args.fc_units = (
            (400 if args.experiment == "splitMNIST" else 2000)
    ...
    if not single_task and not compare_code in ("hyper", "bir"):
        if args.experiment in ["splitM NIST", "fmnist"]:
            ...
            args.si_c = (
                (10.0 if args.scenario == "task" else 0.1)
                if args.si_c is None
                else args.si_c
            )
            args.ewc_lambda = (
                (1000000000.0 if args.scenario == "task" else 100000.0)
            ...
            args.si_c = (
                (100.0 if args.scenario == "task" else 1.0)
                if args.si_c is None
                else args.si_c
            )
            args.ewc_lambda = (
                (1000.0 if args.scenario == "task" else 1.0)
                if args.ewc_lambda is None
                else args.ewc_lambda
            )
            args.gamma = 1 if args.gamma is None else args.gamma
            args.dg_prop = (
                (0.0 if args.scenario == "task" else 0.7)
                if args.dg_prop is None
                else args.dg_prop
            )
            if compare_code == "all":
                args.dg_si_prop = 0.6 if args.dg_si_prop is None else args.dg_si_prop
                args.dg_c = 100000000.0 if args.dg_c is None else args.dg_c
        elif args.experiment == "permMNIST":
            args.si_c = 10.0 if args.si_c is None else args.si_c
            args.ewc_lambda = 1.0 if args.ewc_lambda is None else args.ewc_lambda
            if hasattr(args, "o_lambda"):
                args.o_lambda = 1.0 if args.o_lambda is None else args.o_lambda
            args.gamma = 1.0 if args.gamma is None else args.gamma
            args.dg_prop = 0.8 if args.dg_prop is None else args.dg_prop
            if compare_code == "all":
                args.dg_si_prop = 0.8 if args.dg_si_prop is None else args.dg_si_prop
                args.dg_c = 1.0 if args.dg_c is None else args.dg_c
    ...
    if (not single_task) and (not compare_code in ("hyper")):
        args.xdg_prop = (
            0.0 if args.scenario == "task" and args.xdg_prop is None else args.xdg_prop
        )
...
def check_for_errors(args, single_task=False, **kwargs):
    ...
    if not single_task:
        ...
        if args.scenario == "domain" and checkattr(args, "xdg") and args.xdg_prop > 0:
            print(
                "Although scenario=[domain], 'XdG' makes that task identity is nevertheless always required"
            )
        ...
        if checkattr(args, "only_last") and (not args.replay == "none"):
            raise NotImplementedError(
                "Option 'only_last' is not supported with '{}' replay.".format(
                    args.replay
                )
            )
        if checkattr(args, "only_last") and (
            checkattr(args, "ewc") and args.ewc_lambda > 0
        ):
    ...
    if (
        checkattr(args, "normalize")
        and hasattr(args, "recon_los")
        and args.recon_loss == "BCE"
    ):
        raise ValueError(
            "'BCE' is not a valid reconstruction loss with normalized images"
        )
