...
def run(args):
    ...
    if (utils.checkattr(args, "pre_convE") or utils.checkattr(args, "pre_convD")) and \
        ...
        print("\nDefining the model...")
    cnn = define.define_autoencoder(args=args, config=config, generator=True, device=device)
    ...
    # (Pre)train model
    print("\nTraining...")
    if isinstance(cnn, AutoEncoder):
        train.train(cnn, train_loader, iters, loss_cbs=loss_cbs, eval_cbs=[latent_space_cb],
