...
def _sample_cb(
    ...
    def sample_cb(generator, batch, task=1, epoch=None, allowed_classes=None, **kwargs):
        ...
        if iteration % log == 0:
            statement = " after {} iters{}{}".format(
                batch,
                "" if epoch is None else " (epoch {})".format(epoch),
...
def _latent_space_cb(
    log, datasets, visdom=None, pdf=None, sample_size=400, iters_per_task=None
):
    ...
    def latent_space_cb(model, batch, task=1, **kwargs):
        ...
        if iteration % log == 0:
            ...
            evaluate.visualize_latent_space(
                model,
                X.to(model._device()),
...
def _solver_loss_cb(
    ...
    def cb(bar, iter, loss_dict, task=1, epoch=None):
        ...
        if progress_bar and bar is not None:
            ...
            epoch_stm = (
                ""
                if ((epochs is None) or (epoch is None))
        ...
        if (iteration % log == 0) and (visdom is not None):
            if tasks is None or tasks == 1:
                ...
                current_rnt = (
                    (1.0 / task if (rnt is None) or (task == 1) else rnt)
                    if replay
                    else 1.0
                )
...
def _VAE_loss_cb(
    ...
    def cb(bar, iter, loss_dict, task=1, epoch=None):
        ...
        if (iteration % log == 0) and (visdom is not None):
            ...
            names = list()
            current_rnt = (
                (1.0 / task if (rnt is None or task == 1) else rnt) if replay else 1.0
            )
            ...
            if (
                hasattr(model, "lamda_pl")
                and model.lamda_pl > 0
            ...
            if tasks is not None and replay:
                if tasks > 1:
                    if model.lamda_rcl > 0:
                        plot_data += [
                            (1 - current_rnt) * model.lamda_rcl * loss_dict["recon_r"]
                        ]
                        names += ["Recon - r (x{})".format(model.lamda_rcl)]
                    if model.lamda_vl > 0:
                        plot_data += [
                            (1 - current_rnt) * model.lamda_vl * loss_dict["variat_r"]
                        ]
                    ...
                    if hasattr(model, "lamda_pl") and model.lamda_pl > 0:
                        ...
                        if model.replay_targets == "hard":
                            plot_data += [
                                (1 - current_rnt) * model.lamda_pl * loss_dict["pred_r"]
                            ]
