...
def validate(
    ...
    for data, labels in data_loader:
        ...
        labels = (
            labels - allowed_classes[0] if (allowed_classes is not None) else labels
        )
        with torch.no_grad():
...
def test_accuracy(
    ...
    average_accs = sum(
        [
            accs[task_id] if task_id == 0 else accs[task_id]
    ...
    if visdom is not None:
        visual.visdom.visualize_scalars(
            ...
            names=names,
            iteration=iteration,
            title="Accuracy per task ({})".format(visdom["graph"]),
            env=visdom["env"],
            ylabel="accuracy",
        )
        if n_tasks > 1:
            visual.visdom.visualize_scalars(
                scalars=[average_accs],
                names=["ave accuracy"],
                iteration=iteration,
                title="Average accuracy ({})".format(visdom["graph"]),
                env=visdom["env"],
                ylabel="accuracy",
            )
...
def visualize_latent_space(model, X, y=None, visdom=None, pdf=None, verbose=False):
    ...
    if visdom is not None:
        ...
        visual.visdom.scatter_plot(
            z_tsne,
            title="{} ({})".format(message, visdom["graph"]),
            colors=y + 1 if y is not None else y,
            env=visdom["env"],
        )
...
def show_reconstruction(
    ...
    with torch.no_grad():
        gate_input = (
            (
                torch.tensor(np.repeat(task - 1, size)).to(device)
                if model.dg_type == "task"
                else labels
            )
            if (utils.checkattr(model, "dg_gates") and model.dg_prop > 0)
            else None
        )
        recon_output = model(
            data, gate_input=gate_input, full=True, reparameterize=False
        )
