...
def get_param_stamp_from_args(args):
    ...
    model = (
        define_autoencoder(args=args, config=config, device="cpu")
        if checkattr(args, "feedback")
        else define_classifier(args=args, config=config, device="cpu")
    )
    if checkattr(args, "feedback"):
        model.lamda_pl = 1.0 if not hasattr(args, "pl") else args.pl
    train_gen = (
        hasattr(args, "replay")
...
def get_param_stamp(
    args, model_name, verbose=True, replay=False, replay_model_name=None
):
    ...
    multi_n_stamp = (
        "{n}-{set}{of}".format(
            n=args.tasks,
            set=args.scenario,
            of="OL" if checkattr(args, "only_last") else "",
        )
    ...
    pre_conv = ""
    if (checkattr(args, "pre_convE") or checkattr(args, "pre_convD")) and (
        hasattr(args, "depth") and args.depth > 0
    ):
        ltag = (
            ""
            if not hasattr(args, "convE_ltag") or args.convE_ltag == "none"
            else "-{}".format(args.convE_ltag)
        )
        pre_conv = "-pCvE{}".format(ltag) if args.pre_convE else "-pCvD"
        pre_conv = (
            "-pConv{}".format(ltag)
            if args.pre_convE and checkattr(args, "pre_convD")
            else pre_conv
        )
    freeze_conv = ""
    if (
        (checkattr(args, "freeze_convD") or checkattr(args, "freeze_convE"))
        ...
        freeze_conv = (
            "-fConv"
            if checkattr(args, "freeze_convE") and checkattr(args, "freeze_convD")
            else freeze_conv
        )
    ...
    # -for EWC / SI
    if (checkattr(args, "ewc") and args.ewc_lambda > 0) or (
        checkattr(args, "si") and args.si_c > 0
    ):
        ewc_stamp = (
            "EWC{l}-{fi}{o}".format(
                l=args.ewc_lambda,
                fi="{}".format("N" if args.fisher_n is None else args.fisher_n),
                o="-O{}".format(args.gamma) if checkattr(args, "online") else "",
            )
            if (checkattr(args, "ewc") and args.ewc_lambda > 0)
            else ""
        )
        si_stamp = (
        ...
        both = (
            "--"
            if (checkattr(args, "ewc") and args.ewc_lambda > 0)
            and (checkattr(args, "si") and args.si_c > 0)
            else ""
        )
    ...
    ewc_stamp = (
        ...
        if (
            ...
            or (checkattr(args, "si") and args.si_c > 0)
        )
        else ""
    )
    ...
    if replay:
        replay_stamp = "{H}{rep}{bat}{distil}{model}{gi}{lrs}{seed}".format(
            ...
            bat=""
            if (
                (not hasattr(args, "batch_replay"))
                or (args.batch_replay is None)
                or args.batch_replay == args.batch
            )
            else "-br{}".format(args.batch_replay),
            distil="-Di{}".format(args.temp) if args.distill else "",
            model=""
            if (replay_model_name is None)
            else "-{}".format(replay_model_name),
            gi="-gi{}".format(args.g_iters)
            if (
                hasattr(args, "g_iters")
    ...
    if checkattr(args, "feedback"):
        recon_stamp = "--{}{}".format(
            "H_"
            if checkattr(args, "hidden") and hasattr(args, "depth") and args.depth > 0
            else "",
            args.recon_loss,
        )
