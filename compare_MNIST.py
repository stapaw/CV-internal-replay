...
if __name__ == "__main__":
    ...
    ylabel_all = "Test accuracy ({})".format(
        "based on all digits" if args.scenario == "class" else "after all tasks"
    )
    ylabel = "Test accucary ({} so far)".format(
        "based on digits" if args.scenario == "class" else "on tasks"
    )
    ...
    figure = plt.plot_bar(
        ...
        ylabel=ylabel_all,
        title=title,
        yerr=sems if args.n_seeds > 1 else None,
        ylim=(0, 1),
    )
    ...
    figure = plt.plot_lines(
        ...
        colors=colors,
        title=title,
        xlabel="# {} so far".format("digits" if args.scenario == "class" else "tasks"),
        ylabel=ylabel,
