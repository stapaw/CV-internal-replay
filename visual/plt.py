...
def plot_images_from_tensor(image_tensor, pdf=None, nrow=8, title=None, config=None):
    ...
    )  # pad_value=0 gives black borders
    _ = plt.figure()
...
def plot_bars(
    ...
    if dots is not None:
        maxY = np.max(
            [np.max([np.max(list) for list in list_of_list]) for list_of_list in dots]
        )
    ...
    for i in range(n_plots):
        axarr[i].bar(
            ...
            alpha=alpha[i] if type(alpha) == list else alpha,
        )
        ...
        if vlines is not None:
            axarr[i].axvline(
                x=vlines[i] if type(vlines) == list else vlines,
                color="black",
                linewidth=0.7,
            )
        if dots is not None:
            for j in range(len(dots[i])):
                n_dots = len(dots[i][j])
                axarr[i].plot(
                    [j] * n_dots + np.random.uniform(-0.18, 0.18, size=n_dots),
                    dots[i][j],
                    color="black",
                    marker=".",
                    linewidth=0,
        ...
        # add legend
        if (h_line is not None and h_label is not None) or (
            h_lines is not None and h_labels is not None
        ):
            axarr[i].legend()
        ...
        for task_id, name in enumerate(line_names):
            if errors == "shaded":
                axarr.fill_between(
                    x_axes,
                    list(
                        np.array(list_with_lines[task_id])
                        + np.array(list_with_errors[task_id])
                    ),
                    list(
                        np.array(list_with_lines[task_id])
                        - np.array(list_with_errors[task_id])
...
def plot_matrix(
    ...
    if ylabel is not None:
        axarr.set_ylabel(ylabel)
    axarr.set(
        xticks=np.arange(array.shape[1]),
...
def plot_pr_curves(
    ...
    for group_id in range(len(precision_list)):
        ...
        for line_id in range(n_lines):
            axarr.plot(
                recall_list[group_id][line_id],
                precision_list[group_id][line_id],
                label=None,
                color=colors[group_id] if colors is not None else "black",
                linewidth=2,
                alpha=0.5 * alpha if alpha is not None else 0.5,
                marker="o" if with_dots else None,
            ...
            if new_group:
                ...
                sum_recall = [
                    sum(x) for x in zip(sum_recall, recall_list[group_id][line_id])
                ]
                sum_precision = [
                    sum(x)
                    for x in zip(sum_precision, precision_list[group_id][line_id])
                ]

        # add mean group lines
        axarr.plot(
            [rec / n_lines for rec in sum_recall],
            [pre / n_lines for pre in sum_precision],
