...
def get_gen_results(args):
    ...
    if not os.path.isfile("{}/acc-{}.txt".format(args.r_dir, param_stamp)):
        ...
        "{}/ll-{}.txt".format(args.r_dir, param_stamp)
    ) and os.path.isfile("{}/is{}-{}.txt".format(args.r_dir, eval_tag, param_stamp)):
        print("{}: already run".format(param_stamp))
    else:
...
def barplots(
    ...
    means1 = [
        np.mean([multi * dict1[id][seed][index] for seed in seed_list]) for id in ids
    ]
    if len(seed_list) > 1:
        sems1 = [
            np.sqrt(
                np.var([multi * dict1[id][seed][index] for seed in seed_list])
                / (len(seed_list) - 1)
            )
            for id in ids
        ]
    ...
    means2 = [
        np.mean([multi * dict2[id][seed][index] for seed in seed_list]) for id in ids
    ]
    if len(seed_list) > 1:
        sems2 = [
            np.sqrt(
                np.var([multi * dict2[id][seed][index] for seed in seed_list])
                / (len(seed_list) - 1)
            )
            for id in ids
        ]
    ...
    if only_last_dir is not None:
        ...
        if len(seed_list) > 1:
            h_errors = [
                np.sqrt(
                    np.var([multi * only_last_dir[seed][0] for seed in seed_list])
                    / (len(seed_list) - 1)
                )
            ]
        else:
...
if __name__ == "__main__":
    ...
    if not args.eval_gen:
        ...
        args.only_last = True
        ONLYLAST = {}
        ONLYLAST = collect_all(
            ONLYLAST, seed_list, args, name="Only last task", no_gen=True
        )
    ...
    BIR = collect_all(
        ...
        seed_list,
        args,
        name="Brain-Inspired Replay (BI-R)",
    ...
    BIR_nF = collect_all(
        ...
        seed_list,
        args,
        name="BI-R: no replay-through-feedback",
        no_gen=(not args.eval_gen),
    )
    args.feedback = True
    ...
    BIR_nC = collect_all(
        ...
        seed_list,
        args,
        name="BI-R: no conditional replay",
        no_gen=(not args.eval_gen),
    )
    args.prior = "GMM"
    args.per_class = True
    ...
    BIR_nG = collect_all(
        ...
        seed_list,
        args,
        name="BI-R: no gating based on internal context",
        no_gen=(not args.eval_gen),
    )
    args.dg_gates = True
    ...
    SGR = {}
    SGR = collect_all(
        SGR, seed_list, args, name="Standard GR (s-GR))", no_gen=(not args.eval_gen)
    )
    ...
    SGR_wF = collect_all(
        ...
        seed_list,
        args,
        name="s-GR: with replay-through-feedback",
        no_gen=(not args.eval_gen),
    )
    args.feedback = False
    ...
    SGR_wC = collect_all(
        ...
        seed_list,
        args,
        name="s-GR: with conditional replay",
        no_gen=(not args.eval_gen),
    )
    args.per_class = False
    args.prior = "standard"
    ...
    SGR_wG = collect_all(
        ...
        seed_list,
        args,
        name="s-GR: with gating based on internal context",
        no_gen=(not args.eval_gen),
    )
    args.dg_gates = False
    ...
    SGR_wI = collect_all(
        ...
        seed_list,
        args,
        name="s-GR: with internal replay",
        no_gen=(not args.eval_gen),
    )
    args.hidden = False
    args.freeze_convE = False
    ...
    if not args.eval_gen:
        ##--- AVERAGE ACCURACY ---##
        index = 0
        ylabel = "Test accuracy (after all {} {})".format(
            args.tasks, "episodes" if args.scenario == "class" else "tasks"
        )
        title = "AVERAGE ACCURACY (in %)"
        chance_level = (
            (100.0 / total_classes)
            if args.scenario == "class"
            else (100.0 / int(np.floor(100 / args.tasks)))
        )
        print("\n\n{}\n".format(title) + "-" * 60)
        figure = barplots(
            ALL_sg,
        ...
        ylabel = "Average test negative log-likelihood (over all {})".format(
            "{} classes".format(total_classes)
            if args.scenario == "class"
            else "{} tasks".format(args.tasks)
        )
        ...
        ##--- INCEPTION SCORE ---##
        index = 3
        ylabel = "Modified IS (after all {} {})".format(
            args.tasks, "episodes" if args.scenario == "class" else "tasks"
        )
        ...
        index = 4
        ylabel = "Modified FID (after all {} {})".format(
            args.tasks, "episodes" if args.scenario == "class" else "tasks"
        )
        ...
        fig = pr_curves(
            ...
            i_precision=5,
            title_top=title,
            title="Additions to Standard GR",
            alpha=0.7,
        )
