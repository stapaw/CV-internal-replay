...
class ContinualLearner(nn.Module, metaclass=abc.ABCMeta):
    ...
    def __init__(self):
        ...
        # SI:
        self.si_c = (
            0  # -> hyperparam: how strong to weigh SI-loss ("regularisation strength")
        )
        ...
        for task_id in range(n_tasks):
            ...
            for i in range(self.fcE.layers):
                ...
                gated_units = np.random.choice(
                    n_units, size=int(gating_prop * n_units), replace=False
                )
        ...
        for index, (x, y) in enumerate(data_loader):
            ...
            for label_index in range(output.shape[1]):
                ...
                negloglikelihood = F.cross_entropy(
                    output, label
                )  # --> get neg log-likelihoods for this classs
                ...
                for n, p in self.named_parameters():
                    if p.requires_grad:
                        ...
                        if p.grad is not None:
                            est_fisher_info[n] += label_weights[0][label_index] * (
                                p.grad.detach() ** 2
                            )
