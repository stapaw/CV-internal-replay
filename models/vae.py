...
class AutoEncoder(ContinualLearner):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.dg_gates = dg_gates if dg_prop > 0.0 else False
        self.gate_size = (
            (tasks if dg_type == "task" else classes) if self.dg_gates else 0
        )
        ...
        ##>----Classifier----<##
        if classifier:
            self.units_before_classifier = (
                real_h_dim if self.classify_opt == "beforeZ" else z_dim
            )
            self.classifier = fc_layer(
                self.units_before_classifier, classes, excit_buffer=True, nl="none"
            )
        ...
        if self.dg_gates:
            self.fcD = MLP_gates(
                ...
                if (self.depth == 0 or self.hidden)
                else "normal",
            )
        else:
            self.fcD = MLP(
    ...
    def get_name(self):
        convE_label = (
            "{}{}_".format(self.convE.name, "H" if self.hidden else "")
            if self.depth > 0
            else ""
        )
        fcE_label = (
        ...
        class_label = (
            "_c{}{}".format(
                self.classes,
                "" if self.classify_opt == "beforeZ" else self.classify_opt,
            )
            if hasattr(self, "classifier")
            else ""
        )
        decoder_label = (
            "_{}{}".format("tg" if self.dg_type == "task" else "cg", self.dg_prop)
            if self.dg_gates
            else ""
        )
        return "{}={}{}{}{}{}".format(
    ...
    def layer_info(self):
        ...
        layer_list = (
            self.convE.layer_info(image_size=self.image_size) if not self.hidden else []
        )
    ...
    def classify(self, x, not_hidden=False, reparameterize=True, **kwargs):
        ...
        if hasattr(self, "classifier"):
            image_features = (
                self.flatten(x)
                if (self.hidden and not not_hidden)
                else self.flatten(self.convE(x))
            )
            hE = self.fcE(image_features)
            if self.classify_opt == "beforeZ":
                ...
                z = (
                    mu
                    if (self.classify_opt == "fromZ" or (not reparameterize))
                    else self.reparameterize(mu, logvar)
                )
    ...
    def forward(self, x, gate_input=None, full=False, reparameterize=True, **kwargs):
        ...
        if full:
            ...
            if hasattr(self, "classifier"):
                if self.classify_opt in ["beforeZ", "fromZ"]:
                    y_hat = (
                        self.classifier(hE)
                        if self.classify_opt == "beforeZ"
                        else self.classifier(mu)
                    )
                else:
                    raise NotImplementedError(
                        "Classification-option {} not implemented.".format(
                            self.classify_opt
                        )
                    )
            else:
            ...
            return self.classify(
                x, reparameterize=reparameterize
            )  # -> if [full]=False, only forward pass for prediction
        ...
        **kwargs
        ...
        if self.dg_gates and self.dg_type == "task":
            if self.scenario == "domain":
                ...
                task_used = np.array(
                    [int(class_id / classes_per_task) for class_id in y_used]
                )
        ...
        with torch.no_grad():
            X = self.decode(
                z,
                gate_input=(task_used if self.dg_type == "task" else y_used)
                if self.dg_gates
                else None,
            )
    ...
    def calculate_recon_error(
        self, dataset, batch_size=128, max_batches=None, average=False
    ):
        ...
        if self.scenario == "task" or (
            self.dg_gates and self.dg_prop > 0.0 and self.dg_type == "task"
        ):
            raise NotImplementedError(
        ...
        data_loader = get_data_loader(
            dataset, batch_size=batch_size, cuda=self._is_on_cuda()
        )
        ...
        for index, (x, y) in enumerate(data_loader):
            ...
            recon_error = self.calculate_recon_loss(
                x.view(x.size(0), -1), x_recon.view(x.size(0), -1), average=average
            )
    ...
    def estimate_loglikelihood(self, dataset, S=5000, batch_size=128, max_n=None):
        ...
        for index, (x, y) in enumerate(data_loader):
            ...
            for rep in range(repeats):
                batch_size_current = (
                    (S % batch_size) if rep == (repeats - 1) else batch_size
                )
                ...
                all_lls = (
                    torch.cat([all_lls, log_likelihoods])
                    if rep > 0
                    else log_likelihoods
                )
    ...
    def train_a_batch(
        ...
        **kwargs
        ...
        if x is not None:
            ...
            task_tensor = None
            if self.dg_gates and self.dg_type == "task":
                task_tensor = torch.tensor(np.repeat(task - 1, x.size(0))).to(
                    self._device()
                )
            ...
            recon_batch, y_hat, mu, logvar, z = self(
                x,
                gate_input=(task_tensor if self.dg_type == "task" else y)
                if self.dg_gates
                else None,
                full=True,
                reparameterize=True,
        ...
        if x_ is not None:
            ...
            if (
                (not type(x_) == list)
                and (self.mask_dict is None)
                and (not (self.dg_gates and TaskIL))
            ):
                ...
                # -run full model
                gate_input = (
                    (tasks_ if self.dg_type == "task" else y_predicted)
                    if self.dg_gates
                    else None
                )
            ...
            for replay_id in range(n_replays):
                ...
                if (
                    (type(x_) == list)
                    or (self.mask_dict is not None)
                    or (TaskIL and self.dg_gates)
                ):
                    # -if needed in the decoder-gates, find class-tensor [y_predicted]
                    y_predicted = None
                    if self.dg_gates and self.dg_type == "class":
                    ...
                    # -run full model
                    gate_input = (
                        (tasks_[replay_id] if self.dg_type == "task" else y_predicted)
                        if self.dg_gates
                        else None
                    )
                    recon_batch, y_hat_all, mu, logvar, z = self(
                ...
                (
                    ...
                    z=z,
                    logvar=logvar,
                    allowed_classes=active_classes[replay_id]
                    if active_classes is not None
                    else None,
                )
