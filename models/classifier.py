...
class Classifier(ContinualLearner):
    ...
    def __init__(
        ...
        self.fcE = MLP(
            ...
            gated=fc_gated,
        )  # , output="none") ## NOTE: temporary change!!!
        # --> classifier
        ...
        **kwargs
        ...
        if x is not None:
            ...
            predL = (
                None
                if y is None
                else F.cross_entropy(input=y_hat, target=y, reduction="none")
            )
            # --> no reduction needed, summing over classes is "implicit"
            predL = (
                None if y is None else lf.weighted_average(predL, weights=None, dim=0)
            )  # -> average over batch
            ...
            for replay_id in range(n_replays):
                ...
                if (y_ is not None) and (y_[replay_id] is not None):
                    predL_r[replay_id] = F.cross_entropy(
                        y_hat, y_[replay_id], reduction="none"
                    )
