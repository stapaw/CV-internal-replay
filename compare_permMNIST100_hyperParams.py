...
lamda_list = [0.001, 0.01, 0.1, 1.0, 10.0, 100.0, 1000.0, 10000.0, 100000.0, 1000000.0]
gamma_list = [1.0]
c_list = [0.001, 0.01, 0.1, 1.0, 10.0, 100.0]
gating_list = [0.0, 0.2, 0.4, 0.6, 0.8]
...
def handle_inputs():
    ...
    parser = options.define_args(
        filename="_compare_permMNIST100_hyperParams",
...
if __name__ == "__main__":
    ...
    if len(gamma_list) > 0:
        ...
        for gamma in gamma_list:
            ave_acc_temp = [BASE] + [
                OEWC[gamma][ewc_lambda] for ewc_lambda in lamda_list
            ]
    ...
    # -collect data
    ave_acc_bir = [BIR[dg_prop][0.0] for dg_prop in gating_list]
    # -print on screen
    print("\n\nBRAIN-INSPIRED REPLAY (BI-R)")
    ...
    print(
        "--->  dg_prop = {}     --    {}".format(
            gating_list[np.argmax(ave_acc_bir)], np.max(ave_acc_bir)
        )
    )
    ...
    if args.per_bir_comp:
        ...
        print(
            "--->  dg_prop = {}     --    {}".format(
                gating_list[np.argmax(ave_acc_bir_no_rtf)], np.max(ave_acc_bir_no_rtf)
            )
        )
        ...
        print(
            "--->  dg_prop = {}     --    {}".format(
                gating_list[np.argmax(ave_acc_bir_no_con)], np.max(ave_acc_bir_no_con)
            )
        )
        ...
        print(
            "--->  dg_prop = {}     --    {}".format(
                gating_list[np.argmax(ave_acc_bir_no_dis)], np.max(ave_acc_bir_no_dis)
            )
        )
        ...
        print(
            "--->  dg_prop = {}     --    {}".format(
                gating_list[np.argmax(ave_acc_gr_plus_gat)], np.max(ave_acc_gr_plus_gat)
            )
        )
    print("\n")
    ...
    full_list = (
        [item for sublist in ave_acc_per_lambda for item in sublist]
        + ave_acc_si
    ...
    figure = my_plt.plot_lines(
        ...
        title=title,
        x_log=False,
        xlabel="Context gates: % of nodes gated",
        with_dots=True,
        ylim=(miny - marginy, maxy + marginy),
        h_lines=[BASE],
        h_labels=["None"],
    ...
    # - select colors
    colors = get_cmap("Blues_r")(np.linspace(0.6, 0.0, len(gating_list))).tolist()
    # - make plot (line plot - only average)
    figure = my_plt.plot_lines(
        ave_acc_bir_per_c,
        x_axes=[0] + c_list,
    ...
    if args.per_bir_comp:
        figure = my_plt.plot_lines(
            ...
            colors=["maroon", "red", "green", "darkorange"],
            title=title,
            x_log=False,
            xlabel="Context gates: % of nodes gated",
            with_dots=True,
