...
def get_multitask_experiment(
    ...
    if name == "permMNIST":
        ...
        if not only_config:
            ...
            for labels in labels_per_task:
                ...
                if not only_test:
                    train_datasets.append(
                        SubDataset(
                            fmnist_train, labels, target_transform=target_transform
                        )
                    )
                test_datasets.append(
                    SubDataset(fmnist_test, labels, target_transform=target_transform)
                )
    elif name == "CIFAR100":
        # check for number of tasks
        if tasks > 100:
        ...
        # check for number of tasks
        if tasks > 81:
            raise ValueError("Experiment 'fruits360' cannot have more than 81 tasks!")
        # configurations
