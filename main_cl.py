...
def run(args, verbose=False):
    ...
    if (
        verbose
        and (utils.checkattr(args, "pre_convE") or utils.checkattr(args, "pre_convD"))
    ...
    if args.visdom:
        ...
        replay_statement = (
            "{mode}{fb}{con}{gat}{int}{dis}{b}{u}".format(
                mode=args.replay,
                ...
                if (
                    hasattr(args, "prior")
                    and args.prior == "GMM"
                    and utils.checkattr(args, "per_class")
                )
                else "",
                gat="Gat{}".format(args.dg_prop)
                if (
                    utils.checkattr(args, "dg_gates")
                    and hasattr(args, "dg_prop")
                    and args.dg_prop > 0
                )
                else "",
                ...
                b=""
                if (args.batch_replay is None or args.batch_replay == args.batch)
                else "-br{}".format(args.batch_replay),
                u=""
                if args.g_fc_uni == args.fc_units
                else "-gu{}".format(args.g_fc_uni),
            )
            if (hasattr(args, "replay") and not args.replay == "none")
            else "NR"
        )
        graph_name = "{replay}{syn}{ewc}{xdg}".format(
            ...
            ewc="-ewc{}{}".format(
                args.ewc_lambda,
                "-O{}".format(args.gamma) if utils.checkattr(args, "online") else "",
            )
            if utils.checkattr(args, "ewc")
            else "",
            xdg=""
            if (not utils.checkattr(args, "xdg")) or args.xdg_prop == 0
    ...
    solver_loss_cbs = (
        ...
        if (not utils.checkattr(args, "feedback"))
        else [None]
    )
    ...
    sample_cbs = (
        ...
        if ((train_gen or utils.checkattr(args, "feedback")) and not no_samples)
        else [None]
    )
    ...
    if args.train:
        ...
        train_cl(
            ...
            batch_size_replay=args.batch_replay
            if hasattr(args, "batch_replay")
            else None,
            generator=generator,
            gen_iters=g_iters,
        ...
        if args.save:
            ...
            if generator is not None:
                ...
                utils.save_checkpoint(
                    generator, args.m_dir, name=save_name, verbose=verbose
                )
        ...
        utils.load_checkpoint(
            ...
            verbose=verbose,
            add_si_buffers=(
                isinstance(model, ContinualLearner) and utils.checkattr(args, "si")
            ),
        )
        if generator is not None:
            load_name = (
    ...
    if (
        (utils.checkattr(args, "feedback") or train_gen)
        ...
        # if verbose:
        #     print(
        #         "=> Log-likelihood on test set: {:.4f} +/- {:.4f}\n".format(
        ...
        # -define model
        config["classes"] = 100
        pretrained_classifier = define.define_classifier(
        ...
        try:
            ...
            if verbose:
                print(
                    "= Could not find model {}{} in {}".format(
    ...
    if args.pdf:
        ...
        if args.train and (not utils.checkattr(args, "only_last")):
            ...
            figure = evaluate.visual.plt.plot_lines(
                ...
                x_axes=[i * classes_per_task for i in progress_dict["x_task"]]
                if args.scenario == "class"
                else progress_dict["x_task"],
                line_names=[
                    "{} {}".format(
                ...
                xlabel="# of {}s so far".format(
                    "classe" if args.scenario == "class" else "task"
                ),
            ...
            figure = evaluate.visual.plt.plot_lines(
                ...
                line_names=[
                    "Average based on all {}s so far".format(
                        ("digit" if args.experiment == "splitMNIST" else "classe")
                        if args.scenario
                        else "task"
                    )
                ],
